...
import { Terminal, Code, Play, ChevronLeft, ChevronRight } from 'lucide-react';
...
interface MockResponse {
  ...
  urls: MockData[];       // Changed from items to urls
  total: number;
  current_page: number;   // Changed from page
  page_size: number;
}
...
const MockServerApp: React.FC = () => {
  ...
  const fetchMockUrls = async (page = 1) => {
    try {
      const response = await fetch(`http://localhost:7001/v1/url/query/all?page=${page}&size=${pageSize}`);
      if (!response.ok) throw new Error('Failed to fetch mock URLs');
      const data: MockResponse = await response.json();

      setMockUrls(data.urls);  // Changed from data.items to data.urls
